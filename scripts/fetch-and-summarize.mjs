...
// The first model is configurable through GEMINI_MODEL.
// If it fails after retries, the fallback model is tried.
const GEMINI_MODELS = [
...
function getRetryDelay(response, attempt) {
  ...
  // Exponential backoff with a small amount of randomness:
  // approximately 2s, 4s, 8s, then 16s.
  const baseDelay = 2 ** (attempt - 1) * 2000;
...
async function requestGemini(model, body) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      // Network errors do not have an HTTP status, but they can also be
      // temporary. Retry them using the same backoff strategy.
      if (attempt === maxAttempts) {
        throw new Error(
          `Gemini network error after ${maxAttempts} attempts: ${error.message}`,
        );
      }
      ...
      console.warn(
        `Gemini network error; retrying in ${delayMs / 1000}s ` +
    ...
    // Do not repeatedly retry permanent errors such as an invalid API key.
    if (
      !RETRYABLE_STATUS_CODES.has(response.status) ||
      attempt === maxAttempts
    ) {
      throw new Error(
        `Gemini API error ${response.status} using ${model}: ${errorText}`,
      );
    }
    ...
    console.warn(
      `Gemini model ${model} returned HTTP ${response.status}; ` +
  ...
  for (const model of GEMINI_MODELS) {
    try {
      ...
      if (model !== GEMINI_MODELS[GEMINI_MODELS.length - 1]) {
        console.warn("Trying the fallback Gemini model...");
      }
    }
  }
...
async function main() {
  ...
  const stories = await runEditorialEngine(
    promptTemplate,
    candidates,
    alreadyCovered,
  );
